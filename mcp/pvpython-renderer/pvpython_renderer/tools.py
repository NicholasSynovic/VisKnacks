"""
Shared MCP tool definitions for the ParaView MCP server.

This module defines the single ``FastMCP`` instance and every ``@mcp.tool()``
...
settings) and installs it via :func:`set_pv_manager` before serving.
"""
...
# The ParaView manager is constructed in each engine's ``run()`` (so screenshot
# compression settings can be supplied from the CLI) and installed here via
# set_pv_manager(). It is a module global because the @mcp.tool() functions
# below reference it; it is populated before the MCP server starts serving.
pv_manager: ParaViewManager | None = None

# The single shared FastMCP instance used by both the v1 and v2 engines.
mcp = FastMCP("ParaView", instructions=default_prompt)


def set_pv_manager(manager: ParaViewManager) -> None:
    """Install the ParaView manager referenced by the tool functions."""
    global pv_manager
    pv_manager = manager
...
@mcp.tool()
def save_paraview_state(
    save_directory: str, filename: str = "paraview_state.pvsm"
) -> str:
    """
...
def create_clip(
    ...
    success, message, clip_filter, clip_name = pv_manager.create_clip(
        origin_x, origin_y, origin_z, normal_x, normal_y, normal_z, invert
    )
...
def configure_screenshot_compression(
    enable_compression: bool = None, max_width: int = None, quality: int = None
) -> str:
    """
    ...
    return (
        f"Screenshot settings: compression={'enabled' if pv_manager.compress_screenshots else 'disabled'}, "
...
def set_background_color(
    red: float = 0.32, green: float = 0.34, blue: float = 0.43
) -> str:
    """
...
@mcp.tool()
def get_histogram(
    field: str = None, num_bins: int = 256, data_location: str = "POINTS"
) -> str:
    """
    ...
    """
    success, message, histogram_data = pv_manager.get_histogram(
...
@mcp.tool()
def calculate_field(
    result_name: str, expression: str, attribute_mode: str = "Point Data"
) -> str:
    """
    ...
    Returns:
        Status message
    """
    success, message, calc_filter, calc_name = pv_manager.calculate_field(
        result_name, expression, attribute_mode
    )
    if success:
...
@mcp.tool()
def export_data(
    export_format: str = "csv", filename: str = None, export_type: str = "all"
) -> str:
    """
    ...
    """
    success, message, export_path = pv_manager.export_data(
        export_format, filename, export_type
    )
    return message
...
def create_delaunay3d(
    alpha: float = 0.0, offset: float = 2.0, tolerance: float = 0.001
) -> str:
    """
